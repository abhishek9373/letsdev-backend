...
export class AuthenticationController {
  ...
  async verifyEmail(req: any, res: Response, next: NextFunction) {
    try {
      ...
      res.setHeader('Content-Type', 'text/html');
      const html = "<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><title>Letsdev | verify Email</title><style>.root{display: flex;justify-content: center;}.center{display: flex;flex-direction: column;justify-content: center;align-items: center;}.main-container{margin-top:18rem;background-color: #123182;width: 70vw;padding: 4rem;}.heading{font-family: Inter;font-weight: bold;font-size: 2rem;color: white;}</style></head><body><div class='root'><div class='main-container'><div class='center'><p class='heading' style='margin-bottom: 7rem;'>email verified successfully! you can leave these page</p><a href='https://devbuilder.tech/auth/verifyemail' style='text-decoration:none' class='heading'><button>Go to dashboard</button></a></div></div></div></body></html>"
      return res.end(html);
    } catch (error) {
      next(error);
    }
